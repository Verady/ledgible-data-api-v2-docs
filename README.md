...
**NOTE:** Requests must contain a [signed JWT Bearer token](https://data-api.ledgible.io/api/v2/docs#tag/Authentication) for each request, based on a pre-defined pub/private key pair. The Postman collection below includes a sample pre-request script to assist in generation of these tokens. For this script to work, you will need to create a global variable for the bundle as referenced in the "Manual Setup" section of the pm-lib dependency documentation [here](https://joolfe.github.io/postman-util-lib/). If using a different API client tool or if the script does not meet your organization's requirements, you will need to generate these tokens independently.
